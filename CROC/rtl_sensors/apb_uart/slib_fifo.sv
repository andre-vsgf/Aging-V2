...
module slib_fifo # (parameter WIDTH = 8, parameter SIZE_E=6) (
  ...
  output logic [WIDTH - 1:0] alarm_uart_slib_o,
  `else
  `endif
	output logic		FULL,
...
logic [WIDTH-1:0] fifo_alarm_o;
//logic fifo_alarm_o_comb;

genvar j;

generate
  
  for (j = 0; j < WIDTH; j = j + 1) begin : gen_sensors_bit
      (* dont_touch = "true" *)aging_sensor u_fifo(
    .sclk(CLK),
    .psclk(psclk_uart_i),
    .in_sensor(D[j]),
    .reset(RST),
...
//assign fifo_alarm_o_comb = |fifo_alarm_o;

// ----------------------------
// Final OR para sinal global
// ----------------------------
assign alarm_uart_slib_o = /*|*/fifo_alarm_o;
