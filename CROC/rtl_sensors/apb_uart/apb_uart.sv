...
`ifdef WITH_SENSOR_UART
logic [8-1:0] alarm_uart_slib_o_txff;
logic [11-1:0] alarm_uart_slib_o_rxff;
`else
`endif
...
slib_fifo #(.WIDTH(8), .SIZE_E(6)) UART_TXFF (
  ...
  .alarm_uart_slib_o (alarm_uart_slib_o_txff),
  `else
  `endif
	.EMPTY(iTXFIFOEmpty),
	.FULL(iTXFIFO64Full),
...
slib_fifo #(.WIDTH(11), .SIZE_E(6)) UART_RXFF (
  ...
  .catcher_clk_i (catcher_clk_i),
  .psclk_uart_i (psclk_uart_i),
  .alarm_uart_slib_o (alarm_uart_slib_o_rxff),
  `else
  `endif
	.EMPTY(iRXFIFOEmpty),
	.FULL(iRXFIFO64Full),
...
`ifdef WITH_SENSOR_UART
assign alarm_uart_o = {alarm_uart_slib_o_txff, alarm_uart_slib_o_rxff};
  `else
  `endif
