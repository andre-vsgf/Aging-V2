...
`ifdef WITH_SENSOR_OBI_DMX
  `define WITH_SENSOR_DMX
`else
  `ifdef WITH_SENSOR_OBI_XBAR
    `define WITH_SENSOR_DMX
  `else
  `endif
`endif

`ifdef WITH_SENSOR_DMX
  input logic psclk_dmx_i,
  input logic catcher_clk_i,
  output select_t alarm_dmx_o,
`else
`endif
  input  logic                       clk_i,
  input  logic                       rst_ni,
...
`ifdef WITH_SENSOR_DMX
  genvar i;
  generate
    for (i = 0; i < cf_math_pkg::idx_width(NumMgrPorts); i++) begin : gen_sensors
    (* dont_touch = "true" *)aging_sensor obi_dmx(
    .sclk(clk_i),
    .psclk(psclk_dmx_i),
    .in_sensor(select_d[i]),
    .reset(~rst_ni),
    .clk_en(catcher_clk_i),
    .alarm(),
    .alarm_holder (alarm_dmx_o[i]),
    .xor_out (),
    .ff1 (),
    .ff2 ()
    );
