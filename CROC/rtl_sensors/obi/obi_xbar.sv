...
`ifdef WITH_SENSOR_OBI_DMX
  input logic psclk_dmx_i,
  input logic catcher_clk_i,
  output logic [(cf_math_pkg::idx_width(NumMgrPorts)*NumSbrPorts)-1:0] alarm_dmx_o,
`endif
  ...
  //logic [NumSbrPorts-1:0][cf_math_pkg::idx_width(NumPeriphs)-1:0] alarm_i_obi_demux;
  localparam int unsigned W = cf_math_pkg::idx_width(NumMgrPorts);
  ...
  for (genvar i = 0; i < NumSbrPorts; i++) begin : gen_demux
    ...
    obi_demux #(
      ...
      .rst_ni,
      `ifdef WITH_SENSOR_OBI_DMX
      .psclk_dmx_i   ( psclk_dmx_i ),
      .catcher_clk_i ( catcher_clk_i   ),
      .alarm_dmx_o   ( alarm_dmx_o[i*W +: W] ),
      `else
      `endif
