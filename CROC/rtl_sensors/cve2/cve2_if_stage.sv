...
`else
 `endif
  ...
  /*logic [31:0] instr_rdata_id_i;      //intermediário para ver se funciona

  always_comb begin
    if (if_id_pipe_reg_we)
    instr_rdata_id_i = instr_decompressed;
    else
    instr_rdata_id_i = instr_rdata_id_o;
  end*/
  logic [31:0] instr_rdata_id_muxed;

  assign instr_rdata_id_muxed = if_id_pipe_reg_we ? instr_decompressed : instr_rdata_id_o;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      instr_rdata_id_o         <= instr_rdata_id_muxed;
      if (if_id_pipe_reg_we) begin// To reduce fan-out and help timing from the instr_rdata_id flops they are replicated.
...
`ifdef WITH_SENSOR_AM
  ...
  genvar i;
  generate
  for (i = 0; i < 32; i++) begin : gen_sensors
    (* dont_touch = "true" *)
  aging_sensor u_prefetch(
      .sclk(clk_i),
      .psclk(psclk_f_i),
      .in_sensor(instr_rdata_id_muxed[i]),
      .reset(~rst_ni),
      .clk_en(catcher_clk_i),
      .alarm(),
      .alarm_holder (alarm_f_o[i]),
      .xor_out (xor_out_f_o[i]),
      .ff1 (ff1_f_o[i]),
      .ff2 (ff2_f_o[i])
      );
    end
...
endgenerate
//assign xor_out_f_o = instr_rdata_id_muxed;
`else
 `endif
